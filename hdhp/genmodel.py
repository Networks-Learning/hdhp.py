...
class HDHProcess:
    ...
    def sampleDocuments (self):
        pass

    def generate(self, numUsers, pmatrix, nEvents, reset=True):
        if reset: self.reset ()
       
        # sample the parameters of the generative model
        for i in xrange (numUsers):
            mu_u = self.sample_mu()
            self.mu_per_user[user] = mu_u

        # sample each event one at a time
        self.events = list ()
        for i in xrange (nEvents):
            eTime,eSource = self.sampleNextEvent ()
            eDocs = self.sampleDocuments ()
            self.events.append ((eTime, eDocs, eSource, []))
